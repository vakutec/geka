...
export default function Items({ supabase }: { supabase: ReturnType<typeof createClient> }) {
  ...
  const load = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('items').select('id, name, price_cents, is_active').order('name');
    setLoading(false);
    if (error) return;
    setList(data || []);
  };
  ...
  const add = async () => {
    ...
    const { error } = await supabase.from('items').insert({ name: name.trim(), price_cents: cents, is_active: active });
    if (error) { setErr(error.message); return; }
    setMsg('Artikel angelegt.'); setName(''); setPrice(''); setActive(true); load();
  };
  ...
  return (
    <Card className="rounded-3xl shadow-sm border border-slate-200 bg-white/90 backdrop-blur">
      <CardContent className="p-6 space-y-4">
        <div className="flex items-center gap-2 text-slate-700"><ListOrdered /> <h2 className="font-semibold">Preisliste verwalten</h2></div>
